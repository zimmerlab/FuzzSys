...
from helper_interactive import getMtxSummary, estimateStep, estimateCutoff, getFinalConcept, \
            ...
            ui.card (
                ...
                ui.row (
                    ui.column (
                        4,
                        ui.card (
                            ui.input_file ("crispMatrix", "Select raw value matrix (.TSV):", accept = ".tsv",
                            ...
                            ui.panel_conditional (
                                "input.addNoise === true",
                                ui.input_radio_buttons ("cutoffType", "", choices = {"cons": "Constant cutoffs",
                                                                                     "pct": "Percent cutoffs"},
                                                        selected = "cons", inline = True),
                                ui.panel_conditional (
                                    "input.cutoffType === 'cons'",
                                    ui.input_numeric ("minNoiseLevel", "Values no smaller than:", min = 0, max = 0,
                                                      value = 0, step = 0.01),
                                    ui.input_numeric ("maxNoiseLevel", "Values no larger than:", min = 0, max = 0,
                                                      value = 0, step = 0.01)
                                ),
                                ui.panel_conditional (
                                    "input.cutoffType === 'pct'",
                                    ui.input_numeric ("minNoiseLevelPct", "Percent no smaller than:", min = 0, max = 1,
                                                      value = 0, step = 0.05),
                                    ui.input_numeric ("maxNoiseLevelPct", "Percent no larger than:", min = 0, max = 1,
                                                      value = 1, step = 0.05)
                                )
                            ),
                            ui.layout_columns (
                    ...
                    ui.column (
                        ...
                        ui.navset_card_pill (
                            ...
                            ui.nav_panel (
                                ...
                                ui.layout_columns (
                                    ui.input_slider ("numBins", "Number of bins:", min = 0, max = 100, step = 5, value = 50,
                                                     width = "300px", ticks = True),
                                    ui.input_slider ("zoom", "Visualize range:", min = 0, max = 0, step = 1, value = (0, 0),
                                                     width = "300px")
                                ),
                                ui.div (
                ...
                ui.navset_pill (
                    ui.nav_panel (
                        ...
                        ui.layout_sidebar (
                            ...
                            ui.layout_columns (
                                ...
                                ui.input_selectize ("viewFeature_cons", "", choices = {"ALL": "ALL"}, multiple = False,
                                                    remove_button = True),
                                ui.div ()
                            ),
                            ui.layout_columns (
                                "Number of bins:",
                                ui.input_slider ("numBins_cons", "", min = 0, max = 100, step = 5, value = 50, width = "300px",
                                                 ticks = True),
                                ui.div ()
                            ),
                            ui.div (
                    ...
                    ui.nav_panel (
                        ...
                        ui.layout_sidebar (
                            ...
                            ui.layout_columns (
                                "Select feature for visualization:",
                                ui.input_selectize ("viewFeature_fit", "", choices = {"ALL": "ALL"}, multiple = False,
                                                    remove_button = True),
                                ui.div ()
                            ),
                            ui.layout_columns (
                                "Number of bins:",
                                ui.input_slider ("numBins_fit", "", min = 0, max = 100, step = 5, value = 50, width = "300px",
                                                 ticks = True),
                                ui.div ()
                            ),
                            ui.div (
        ...
        ui.nav_panel (
            ...
            ui.card (
                ...
                ui.row (
                    ui.column (
                        5,
                        ui.input_radio_buttons ("useMatrix", "", choices = {"original": "Use original matrix",
                                                                            "upload": "Upload new matrix"},
                                                selected = "upload", inline = True),
                        ui.panel_conditional (
                            "input.useMatrix === 'upload'",
                            ui.input_file ("crispMatrix_visual", "Select raw value matrix (.TSV):", accept = ".tsv",
                                           multiple = False, width = "80%")
                        ),
                        height = "200px"
                    ),
                    ...
                    ui.column (
                        5,
                        ui.input_radio_buttons ("useConcept", "", choices = {"original": "Use original concepts",
                                                                             "upload": "Upload new concepts"},
                                                selected = "upload", inline = True),
                        ui.panel_conditional (
                            "input.useConcept === 'original'",
                            ui.input_radio_buttons ("definedBy", "Use fuzzy concepts defined by:",
                                                    choices = {"cons": "constraints", "fit": "fitting"},
                                                    selected = "cons", inline = True)
                        ),
                        ui.panel_conditional (
                            "input.useConcept === 'upload'",
                            ui.input_file ("concept_visual", "Select fuzzy concepts (.JSON):", accept = ".json",
                                           multiple = False, width = "80%")
                        ),
                        height = "200px"
                    )
            ...
            ui.card (
                ...
                ui.layout_sidebar (
                    ui.sidebar (
                        ...
                        ui.br (),
                        ui.input_selectize ("viewFeature_raw_visual", "", choices = {"ALL": "ALL"}, selected = "ALL",
                                            multiple = False, remove_button = True),
                        ui.br (),
                        "Select minimal and maximal value range:",
                        ui.input_slider ("zoom_visual", "", min = 0, max = 0, step = 1, value = (0, 0), width = "300px"),
                        ui.input_slider ("numBins_visual", "Number of bins:", min = 0, max = 100, step = 5, value = 50,
                                         width = "300px", ticks = True),
                        "Select feature from fuzzy concepts:",
                        ui.br (),
                        ui.input_selectize ("viewFeature_concept_visual", "", choices = {"--": "--"}, selected = "--",
                                            multiple = False, remove_button = True),
                        ui.br (),
                        ui.input_slider ("deviationCutoff", "Maximal deviation:", min = 0, max = 1, step = 0.05, value = 0.2,
                                         width = "300px", ticks = True),
                        width = "400px", position = "left", open = "open"
                    ),
                    ui.div (
